...
const QUESTION_COMBINATIONS: {
  id: TestQuestionTypeCombination;
  title: string;
  description: string;
  badge: string;
}[] = [
  {
    id: 'mcqs_only',
    title: 'MCQs only',
    description: 'Section A only with objective multiple-choice questions and bubble key.',
    badge: 'Objective',
  },
  {
    id: 'short_only',
    title: 'Short Questions only',
    description: 'Section B only with conceptual and analytical short-answer questions.',
    badge: 'Subjective',
  },
  {
    id: 'long_only',
    title: 'Long Questions only',
    description: 'Section C only with comprehensive theoretical derivations and problems.',
    badge: 'Comprehensive',
  },
  {
    id: 'mcqs_and_short',
    title: 'MCQs + Short Questions',
    description: 'Standard mid-term format with Section A (MCQs) and Section B (Short Questions).',
    badge: 'Popular',
  },
  {
    id: 'mcqs_and_long',
    title: 'MCQs + Long Questions',
    description: 'High-weightage format with Section A (MCQs) and Section C (Detailed Questions).',
    badge: 'Mixed',
  },
  {
    id: 'all_types',
    title: 'MCQs + Short Questions + Long Questions',
    description: 'Complete formal board exam paper with Section A, Section B, and Section C.',
    badge: 'Full Board Paper',
  },
];
...
export const AdminCreateTestModal: React.FC<AdminCreateTestModalProps> = ({
  ...
  // Question Combination & Section Config
  const [combination, setCombination] = useState<TestQuestionTypeCombination>('all_types');
  ...
  const totalCalculatedMarks = useMemo(() => {
    let total = 0;
    if (combination === 'mcqs_only' || combination === 'mcqs_and_short' || combination === 'mcqs_and_long' || combination === 'all_types') {
      total += mcqCount * mcqMarksEach;
    }
    if (combination === 'short_only' || combination === 'mcqs_and_short' || combination === 'all_types') {
      total += shortAttemptCount * shortMarksEach;
    }
    if (combination === 'long_only' || combination === 'mcqs_and_long' || combination === 'all_types') {
      total += longAttemptCount * longMarksEach;
    }
    return total;
  }, [combination, mcqCount, mcqMarksEach, shortAttemptCount, shortMarksEach, longAttemptCount, longMarksEach]);
  ...
  const handlePullQuestions = async () => {
    ...
    try {
      ...
      setPulledMCQs(res.mcqs);
      setPulledShortQuestions(res.shortQuestions);
      setPulledLongQuestions(res.longQuestions);
      toast.success(
        `Pulled ${res.mcqs.length} MCQs, ${res.shortQuestions.length} Short Questions, ${res.longQuestions.length} Long Questions.`
      );
    } catch (err: any) {
  ...
  const handleGeneratePdfPreview = async () => {
    ...
    try {
      const spec: GeneratedTestSpecification = {
        ...
        combination,
        mcqs: pulledMCQs,
        shortQuestions: pulledShortQuestions,
        longQuestions: pulledLongQuestions,
        mcqMarksEach,
        shortMarksEach,
        shortAttemptCount,
        longMarksEach,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs overflow-y-auto">
      <div className="bg-white rounded-2xl w-full max-w-4xl shadow-2xl border border-[#E5E5E5] flex flex-col max-h-[92vh] overflow-hidden my-auto">
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-5">
          ...
          {/* STEP 2: QUESTION TYPE COMBINATION SELECTION */}
          {step === 2 && (
            <div className="space-y-5">
              <div>
                <label className="block text-sm font-black text-[#111111] mb-1">
                  Select Question Types Combination
                </label>
                <p className="text-xs text-[#737373]">
                  Choose which question types to include in this test paper. Questions will be pulled directly from the corresponding Question Banks.
                </p>
              </div>

              {/* The 6 Combinations Grid */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {QUESTION_COMBINATIONS.map((c) => {
                  const isSelected = combination === c.id;
                  return (
                    <button
                      key={c.id}
                      onClick={() => setCombination(c.id)}
                    ...
                    >
                      ...
                      <div>
                        <div className={`text-sm font-black ${isSelected ? 'text-white' : 'text-[#111111]'}`}>
                          {c.title}
                        </div>
                        <p
                          className={`text-xs mt-1 line-clamp-2 ${
                            isSelected ? 'text-white/80' : 'text-[#737373]'
                          }`}
                        >
                          {c.description}
                        </p>
            ...
            <div className="space-y-4">
              ...
              {pullingQuestions ? (
                ...
                <div className="space-y-5 max-h-[50vh] overflow-y-auto pr-1">
                  ...
                  {pulledMCQs.length > 0 && (
                    <div className="space-y-2">
                      <div className="text-xs font-black uppercase text-[#111111] flex items-center justify-between bg-[#F5F5F5] p-2 rounded-lg">
                        <span>Section A: {pulledMCQs.length} MCQs ({pulledMCQs.length * mcqMarksEach} Marks)</span>
                  ...
                  {pulledShortQuestions.length > 0 && (
                    <div className="space-y-2">
                      <div className="text-xs font-black uppercase text-[#111111] flex items-center justify-between bg-[#F5F5F5] p-2 rounded-lg">
                        <span>Section B: {pulledShortQuestions.length} Short Questions (Attempt {shortAttemptCount} × {shortMarksEach} = {shortAttemptCount * shortMarksEach} Marks)</span>
                      </div>
                      <div className="space-y-2">
                  ...
                  {pulledLongQuestions.length > 0 && (
                    <div className="space-y-2">
                      <div className="text-xs font-black uppercase text-[#111111] flex items-center justify-between bg-[#F5F5F5] p-2 rounded-lg">
                        <span>Section C: {pulledLongQuestions.length} Long Questions (Attempt {longAttemptCount} × {longMarksEach} = {longAttemptCount * longMarksEach} Marks)</span>
                      </div>
                      <div className="space-y-2">

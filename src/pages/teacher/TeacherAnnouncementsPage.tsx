...
import { Megaphone, Calendar, Plus, Trash2, Sparkles, CheckCircle2, AlertTriangle } from 'lucide-react';
...
export const TeacherAnnouncementsPage: React.FC = () => {
  ...
  const [selectedOfferingId, setSelectedOfferingId] = useState(teacherOfferingIds[0] || '');
  ...
  const handleAddAnnouncement = (e: React.FormEvent) => {
    ...
    if (!selectedOfferingId) {
      triggerNotification('error', 'Please select a class offering.');
      return;
    }
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <SectionHeader
            ...
            description="Broadcast guidelines, class updates, and homework notices directly to your students."
          />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className="card bg-white border border-[#E5E5E5] rounded-2xl p-5 sticky top-24 shadow-sm">
              <div className="flex items-center gap-2 mb-4 border-b border-[#F5F5F5] pb-3">
                <Sparkles size={18} className="text-[#F4C430]" />
                <h2 className="font-bold text-[#111111] text-base">New Class Broadcast</h2>
              </div>

              {teacherOfferings.length === 0 ? (
                <p className="text-xs text-[#737373] text-center py-4 font-semibold">
                  You are not assigned to any classes to publish announcements.
                </p>
              ) : (
                <form onSubmit={handleAddAnnouncement} className="space-y-4">
                  <div>
                    <label className="label text-xs font-bold text-[#737373] uppercase tracking-wide mb-1.5 block">Target Class Offering</label>
                    <select
                    ...
                    >
                      {teacherOfferings.map((o) => (
                        <option key={o.id} value={o.id}>
                          {o.subject} (Grade {o.grade} · {o.board.toUpperCase()})
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="e.g. Next class starting location"
                      value={title}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      >
                        ...
                        <option value="⚠️">⚠️ Notice</option>
              ...
              {visibleAnnouncements.length === 0 ? (
                ...
                <div className="space-y-4">
                  {visibleAnnouncements.map((ann) => {
                    ...
                    // Teacher can delete announcements they posted (i.e. those scoped to their offerings)
                    const canDelete = ann.offering_id && teacherOfferingIds.includes(ann.offering_id);
                    return (
                      <div
                        key={ann.id}

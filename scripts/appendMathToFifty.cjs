/**
 * appendMathToFifty.cjs
 * 
 * Appends 10 new verified MCQs per Mathematics chapter (40 -> 50),
...
const part1 = require('./data/mathFiftyPart1.cjs');
const part2 = require('./data/mathFiftyPart2.cjs');
...
console.log('Verifying chapters and preparing append (40 -> 50 MCQs)...');
...
for (const ch of chapters) {
  ...
  if (existingQs.length !== 40) {
    console.warn(`Warning: Expected 40 existing questions for ${ch}, found ${existingQs.length}`);
  }
...
console.log('✓ Successfully written src/data/banks/mathematics.json with 50 MCQs/chapter');
...
for (const ch of chapters) {
  ...
  if (!qs || qs.length !== 50) {
    throw new Error(`Verification failed for Mathematics chapter ${ch}: expected 50, found ${qs ? qs.length : 0}`);
  }
  qs.forEach((q, idx) => {
...
console.log('✓ ALL 11 Mathematics chapters verified with EXACTLY 50 MCQs each (550 total)!');

/**
 * appendMathToThirty.cjs
 * 
 * Appends 10 new verified MCQs per Mathematics chapter (20 -> 30),
...
const part3 = require('./data/mathTenMorePart3.cjs');
const part4 = require('./data/mathTenMorePart4.cjs');
...
console.log('Verifying chapters and preparing append (20 -> 30 MCQs)...');
...
for (const ch of chapters) {
  ...
  if (existingQs.length !== 20) {
    console.warn(`Warning: Expected 20 existing questions for ${ch}, found ${existingQs.length}`);
  }
...
console.log('✓ Successfully written src/data/banks/mathematics.json with 30 MCQs/chapter');
...
for (const ch of chapters) {
  ...
  if (!qs || qs.length !== 30) {
    throw new Error(`Verification failed for Mathematics chapter ${ch}: expected 30, found ${qs ? qs.length : 0}`);
  }
  qs.forEach((q, idx) => {
...
console.log('✓ ALL 11 Mathematics chapters verified with EXACTLY 30 MCQs each (330 total)!');

/**
 * appendMathTenMore.cjs
 * 
 * Appends 10 new verified MCQs per Mathematics chapter (10 -> 20),
...
const part1 = require('./data/mathTenMorePart1.cjs');
const part2 = require('./data/mathTenMorePart2.cjs');
...
console.log('Verifying chapters and preparing append...');
...
for (const ch of chapters) {
  ...
  if (existingQs.length !== 10) {
    console.warn(`Warning: Expected 10 existing questions for ${ch}, found ${existingQs.length}`);
  }
...
// Let's ensure mathematics.json contains the direct chapter dictionary
...
console.log('✓ Successfully written src/data/banks/mathematics.json with 20 MCQs/chapter');
...
for (const ch of chapters) {
  ...
  if (!qs || qs.length !== 20) {
    throw new Error(`Verification failed for Mathematics chapter ${ch}: expected 20, found ${qs ? qs.length : 0}`);
  }
  qs.forEach((q, idx) => {
...
console.log('✓ ALL 11 Mathematics chapters verified with EXACTLY 20 MCQs each (220 total)!');

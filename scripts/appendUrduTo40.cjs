...
const batch1 = require('./data/urduAppend10Batch4Part1.cjs');
const batch2 = require('./data/urduAppend10Batch4Part2.cjs');
const batch3 = require('./data/urduAppend10Batch4Part3.cjs');
const batch4 = require('./data/urduAppend10Batch4Part4.cjs');
...
console.log(`Loaded ${newChapterKeys.length} chapters in new batch (Questions 31-40).`);
...
for (const chName of existingKeys) {
  ...
  if (!existingList || existingList.length !== 30) {
    throw new Error(`Chapter "${chName}" in existing bank does not have 30 MCQs (has ${existingList ? existingList.length : 0})`);
  }
  ...
  newList.forEach((q, idx) => {
    if (existingQuestionsSet.has(q.question.trim())) {
      throw new Error(`Duplicate question found in "${chName}" (new Q${idx + 31}): "${q.question}"`);
    }
    ...
    if (!q.id || !q.question || !q.options || !q.options.A || !q.options.B || !q.options.C || !q.options.D || !['A', 'B', 'C', 'D'].includes(q.correctAnswer) || !q.explanation) {
      throw new Error(`Invalid question structure in "${chName}" Q${idx + 31}: ${JSON.stringify(q)}`);
    }
  });

  // Combine (strict append)
  const combinedList = [...existingList, ...newList];
  if (combinedList.length !== 40) {
    throw new Error(`Combined list for "${chName}" does not have 40 MCQs (has ${combinedList.length})`);
  }
...
if (totalCombinedMCQs !== 880) {
  throw new Error(`Expected 880 total Urdu MCQs, got ${totalCombinedMCQs}`);
}

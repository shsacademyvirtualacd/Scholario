...
const batch1 = require('./data/urduAppend10Batch3Part1.cjs');
const batch2 = require('./data/urduAppend10Batch3Part2.cjs');
const batch3 = require('./data/urduAppend10Batch3Part3.cjs');
const batch4 = require('./data/urduAppend10Batch3Part4.cjs');
...
console.log(`Loaded ${newChapterKeys.length} chapters in new batch (Questions 21-30).`);
...
for (const chName of existingKeys) {
  ...
  if (!existingList || existingList.length !== 20) {
    throw new Error(`Chapter "${chName}" in existing bank does not have 20 MCQs (has ${existingList ? existingList.length : 0})`);
  }
  ...
  newList.forEach((q, idx) => {
    if (existingQuestionsSet.has(q.question.trim())) {
      throw new Error(`Duplicate question found in "${chName}" (new Q${idx + 21}): "${q.question}"`);
    }
    ...
    if (!q.id || !q.question || !q.options || !q.options.A || !q.options.B || !q.options.C || !q.options.D || !['A', 'B', 'C', 'D'].includes(q.correctAnswer) || !q.explanation) {
      throw new Error(`Invalid question structure in "${chName}" Q${idx + 21}: ${JSON.stringify(q)}`);
    }
  });

  // Combine (strict append)
  const combinedList = [...existingList, ...newList];
  if (combinedList.length !== 30) {
    throw new Error(`Combined list for "${chName}" does not have 30 MCQs (has ${combinedList.length})`);
  }
...
if (totalCombinedMCQs !== 660) {
  throw new Error(`Expected 660 total Urdu MCQs, got ${totalCombinedMCQs}`);
}

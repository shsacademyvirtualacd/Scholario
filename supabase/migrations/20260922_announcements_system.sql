...
-- Migration: 20260922_announcements_system.sql
-- 
-- 1. Upgrades public.announcements table to support:
--    - announcement_type ('public' | 'dashboard')
--    - target_roles (text[]: e.g. ARRAY['student'], ARRAY['teacher'], ARRAY['all'])
--    - is_active (boolean)
--    - starts_at / ends_at (timestamptz for time-limited campaigns)
--    - action_label & action_url (for CTA buttons)
--    - badge_label (e.g. 'Admissions Open', 'New Feature', 'Student Guide')
-- 2. Creates public.announcement_dismissals table for per-account "Don't show again" tracking
-- 3. Row Level Security (RLS) policies for anonymous visitors, students, teachers, and admins
-- 4. Initial Seed Announcements (Public Admissions Announcement & Role Onboarding Manuals)
-- ═══════════════════════════════════════════════════════════════════════════

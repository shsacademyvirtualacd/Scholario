...
async function run() {
  ...
  // Parse connection string and fix password encoding if needed
  connectionString = connectionString.replace('Marcelmmm23155@@', 'Marcelmmm23155%40@');

  console.log("Connecting with DB URL...");
  
  // Try connecting via direct or pooler host
  ...
  try {
    ...
    // 2. CLASSES
    console.log("\n--- 2. CLASSES ---");
    const classesRes = await client.query(`SELECT * FROM public.classes ORDER BY grade;`);
    ...
    // 8. PROFILES FOR GRADE 9 AND 10 STUDENTS
    ...
    console.error("Error executing pg query:", err);
  } finally {
    await client.end();
  }
}
